...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $hadith_text = $_POST['hadith_text'] ?? '';
    $source = $_POST['source'] ?? '';
    if ($narrator && $hadith_text && $source) {
        $stmt = $conn->prepare('INSERT INTO hadith (narrator, hadith_text, source) VALUES (?, ?, ?)');
        $stmt->bind_param('sss', $narrator, $hadith_text, $source);
        if ($stmt->execute()) {
            $msg = 'হাদীস সফলভাবে যোগ হয়েছে!';
        } else {
...
<body>
    <div class="form-box">
        <h2>হাদীস যোগ করুন</h2>
        <?php if ($msg): ?>
            <div class="<?php echo strpos($msg, 'সফলভাবে') !== false ? 'msg' : 'err'; ?>"><?php echo $msg; ?></div>
        <?php endif; ?>
        <form method="post">
            <label for="narrator">বর্ণনাকারী</label>
            <input type="text" id="narrator" name="narrator" required>
            <label for="hadith_text">হাদীস</label>
            <textarea id="hadith_text" name="hadith_text" required></textarea>
            <label for="source">উৎস</label>
